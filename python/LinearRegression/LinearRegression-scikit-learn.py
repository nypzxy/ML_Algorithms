# -*- coding: utf-8 -*-
from __future__ import print_function
import numpy as np
from sklearn import linear_model
from sklearn.preprocessing import StandardScaler  # 引入归一化的包


def linearRegression():
    print(u"加载数据...\n")
    data = loadtxtAndcsv_data("data.txt", ",", np.float32)
    X = np.array(data[:, 0:-1], dtype=np.float32)
    y = np.array(data[:, -1], dtype=np.float32)

    # 进行归一化操作
    ss = StandardScaler()
    x_train = ss.fit_transform(X)  # fit相当于训练  transform相当于归一化
    x_test = ss.transform(np.array([[1650, 3]]))  # [[1650, 3]] 这是二维 shape值是1,2 [1650, 3]这个是1维 shape值是2,

    # 线性模型
    model = linear_model.LinearRegression()
    model.fit(x_train, y)

    # 预测结果
    result = model.predict(x_test)
    print(model.coef_)  # Coefficient of the features 决策函数中的特征系数
    print(model.intercept_)  # 又名bias偏置,若设置为False，则为0
    print(result)  # 预测结果


# 加载txt和csv文件
def loadtxtAndcsv_data(fileName, split, dataType):
    return np.loadtxt(fileName, delimiter=split, dtype=dataType)


if __name__ == "__main__":
    linearRegression()
