...
# 数据加载
mnist = input_data.read_data_sets('data/', one_hot=True)  # 55000,784  (784 特征)   label 10列=>亚编码
...
# 定义输入的占位符  为什么用占位符 不写死  其实我也不知道？  但是这么做也可以吧， 这个可以再session中run里面指定
x = tf.placeholder(tf.float32, shape=[None, n_input], name='x')
y = tf.placeholder(tf.float32, shape=[None, n_classes], name='y')

# 构建初始化的w和b  因为有784维特征 也就是784个输入  第一层是256个op  所以我们的w1对应的自然就是256个w11 w12 ... w1(256)的值喽 每个特征都会对应着256个op
# 所以 第一层的op的模型是784*256的矩阵  那么通过激活函数是256个概率值 第二层是128个  所以w2对应 256*128的矩阵
# 最后out 是128个输出的概率 到10个op  128*10
weights = {
...
def neural_network_build_up(_X, _weights, _biases):
...
# 获取预测值
act = neural_network_build_up(x, weights, biases)
